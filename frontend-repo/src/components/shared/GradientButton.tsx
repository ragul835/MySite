...
interface GradientButtonProps extends Omit<React.ButtonHTMLAttributes<HTMLButtonElement>, "onClick" | "className" | "children"> {
  href?: string;
...
export function GradientButton({ href, children, className, onClick, type = "button", ...props }: GradientButtonProps) {
  ...
  return (
    <button
      ...
      onClick={handleClick}
      className={cn(
        "group relative inline-flex items-center justify-center px-8 py-3.5 text-sm font-semibold text-white transition-all duration-300 bg-gradient-to-r from-primary to-secondary rounded-lg shadow-[0_0_20px_hsl(var(--primary)/0.3)] hover:scale-105 active:scale-95 hover:shadow-[0_0_40px_hsl(var(--primary)/0.6)] hover:brightness-110 border border-white/20 overflow-hidden",
        className
      )}
      {...props}
    >
